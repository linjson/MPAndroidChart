...
import com.github.mikephil.charting.sharechart.kline.KLineData;
import com.github.mikephil.charting.utils.ColorTemplate;
import com.github.mikephil.charting.utils.Transformer;
import com.github.mikephil.charting.utils.ViewPortHandler;

/**
 * Created by ljs on 16/4/21.
 */
public class MarketKlineChartRender extends CandleStickChartRenderer {
    public MarketKlineChartRender(CandleDataProvider chart, ChartAnimator animator, ViewPortHandler viewPortHandler) {
    ...
    protected void drawDataSet(Canvas c, ICandleDataSet dataSet) {
        ...
        for (int j = minx,
            ...
            KLineData kData = (KLineData) e.getData();
            ...
            if (showCandleBar) {
                ...
                if (dataSet.getShadowColorSameAsCandle()) {
                    ...
                    if (open > close)
                        preDataColor = setupDecreasingColor(dataSet, j);

                    else if (open < close)
                        preDataColor = setupIncreasingColor(dataSet, j);

                    else {

                        KLineData preData = kData.preKLineData;

                        if (preData != null) {

                            if (close > preData.close) {
                                preDataColor = setupIncreasingColor(dataSet, j);
                            } else if (close < preData.close) {
                                preDataColor = setupDecreasingColor(dataSet, j);
                            } else {
                                mRenderPaint.setColor(preDataColor);
                            }
...
//                            );
                ...
                if (open > close) { // decreasing
                    ...
                    if (preData != null) {
                        ...
                        if (close > preData.close) {
                            setupIncreasingColor(dataSet,j);
                            mRenderPaint.setStyle(dataSet.getIncreasingPaintStyle());
...
//                    }
    ...
    private int setupDecreasingColor(ICandleDataSet dataSet, int j) {
        mRenderPaint.setColor(
                dataSet.getDecreasingColor() == ColorTemplate.COLOR_NONE ?
                        dataSet.getColor(j) :
                        dataSet.getDecreasingColor()
        );
        return mRenderPaint.getColor();
    }

    private int setupIncreasingColor(ICandleDataSet dataSet, int j) {
        mRenderPaint.setColor(
